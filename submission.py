...
if args.dataset == '2015':
    ...
    test_left_img, test_right_img, _ = DA.dataloader(args.datapath, 2178, 2500)
else:
    from dataloader import kitti15list as DA
...
model = VCN([1, maxw, maxh], md=[int(4 * (args.maxdisp / 256)), 4, 4, 4, 4], fac=args.fac,
            exp_unc=not ('kitti' in args.loadmodel))
...
def main():
    ...
    for inx in range(len(test_left_img)):
        ...
        if '2015' in args.dataset:
            ...
            K0, _ = cam_read('/data/gengshay/tf_depth/sintel-data/training/camdata_left/%s_%s/frame_%04d.cam' % (
            seqname1, seqname2, framename + 1))
            K1, _ = cam_read('/data/gengshay/tf_depth/sintel-data/training/camdata_left/%s_%s/frame_%04d.cam' % (
