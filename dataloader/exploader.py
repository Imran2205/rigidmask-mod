...
class myImageFloder(data.Dataset):
    def __init__(self, iml0, iml1, flowl0, loader=default_loader, dploader=flow_loader, scale=1., shape=[320, 448],
                 order=1, noise=0.06, pca_augmentor=True, prob=1., disp0=None, disp1=None, calib=None):
        self.iml0 = iml0
        self.iml1 = iml1
        self.flowl0 = flowl0
        self.loader = loader
    ...
    def __getitem__(self, index):
        ...
        if 'kitti_scene' in self.iml0[index]:
            ...
            obj_fname = '%s/%04d.pfm' % (
            self.flowl0[index].replace('/optical_flow', 'object_index').replace('into_past/', '/').replace(
                'into_future/', '/').rsplit('/', 1)[0], fnum)
