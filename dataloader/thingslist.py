...
def dataloader(filepath):
    ...
    l1_trainlf = ['%s/%s.png' % (img.rsplit('/', 1)[0], '%07d' % (1 + int(img.split('.')[0].split('/')[-1]))) for img in
                  l0_trainlf]
    ...
    l1_trainrf = ['%s/%s.png' % (img.rsplit('/', 1)[0], '%07d' % (1 + int(img.split('.')[0].split('/')[-1]))) for img in
                  l0_trainrf]
    ...
    l1_trainlp = ['%s/%s.png' % (img.rsplit('/', 1)[0], '%07d' % (-1 + int(img.split('.')[0].split('/')[-1]))) for img
                  in l0_trainlp]
    ...
    l1_trainrp = ['%s/%s.png' % (img.rsplit('/', 1)[0], '%07d' % (-1 + int(img.split('.')[0].split('/')[-1]))) for img
                  in l0_trainrp]
