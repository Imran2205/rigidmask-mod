...
def dataloader(filepath):
    ...
    l0_train = [img for img in l0_train if '%s_%s.png' % (
    img.rsplit('_', 1)[0], '%04d' % (1 + int(img.split('.')[0].split('_')[-1]))) in l0_train and ('000035' not in img)]
    l1_train = ['%s_%s.png' % (img.rsplit('_', 1)[0], '%04d' % (1 + int(img.split('.')[0].split('_')[-1]))) for img in
                l0_train]
