...
class RandomCrop(object):
    ...
    def __call__(self, inputs, target, intr):
        ...
        return inputs, target[y1: y1 + th, x1: x1 + tw].astype(float), list(np.asarray(intr).astype(float)) + list(
            np.asarray([1., 0., 0., 1., 0., 0.]).astype(float))
...
class SpatialAug(object):
    ...
    def __call__(self, inputs, target, intr):
        ...
        inputs_0 = \
        F.grid_sample(torch.Tensor(inputs[0]).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], align_corners=True)[
            0].permute(1, 2, 0)
        if self.order == 0:
            target_0 = \
            ...
            target_0 = \
            F.grid_sample(torch.Tensor(target).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], align_corners=True)[
                0].permute(1, 2, 0)

        # object index
        target_0[:, :, -1] = \
        F.grid_sample(torch.Tensor(target[:, :, -1:]).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], mode='nearest',
                      align_corners=True)[0].permute(1, 2, 0)[:, :, -1]
        # sparse depth
        if target[:, :, 3].max() == np.inf:
            target_0[:, :, 3] = \
            F.grid_sample(torch.Tensor(target[:, :, 3:4]).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis],
                          mode='nearest', align_corners=True)[0].permute(1, 2, 0)[:, :, 0]

        mask_0 = target[:, :, 2:3].copy();
        mask_0[mask_0 == 0] = np.nan
        if self.order == 0:
            mask_0 = F.grid_sample(torch.Tensor(mask_0).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], mode='nearest',
                                   align_corners=True)[0].permute(1, 2, 0)
        else:
            mask_0 = \
            F.grid_sample(torch.Tensor(mask_0).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], align_corners=True)[
                0].permute(1, 2, 0)
        mask_0[torch.isnan(mask_0)] = 0

        vgrid = self.grid_transform(meshgrid, transmat1, gridsize=[float(h), float(w)])
        inputs_1 = \
        F.grid_sample(torch.Tensor(inputs[1]).permute(2, 0, 1)[np.newaxis], vgrid[np.newaxis], align_corners=True)[
            0].permute(1, 2, 0)
