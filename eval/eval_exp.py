...
def eval_f(fp):
    ...
    # pred
    logexp = disparity_loader(
        '%s/%s/exp-%s.pfm' % (args.path, args.dataset, expansionp[fp].split('/')[-1].split('.')[0]))
    logexp = np.clip(logexp, -np.log(2), np.log(2))
    logexp_error = np.abs(gt_logexp - logexp)[oemask].mean()

    logdc = disparity_loader(
        '%s/%s/mid-%s.pfm' % (args.path, args.dataset, expansionp[fp].split('/')[-1].split('.')[0]))
