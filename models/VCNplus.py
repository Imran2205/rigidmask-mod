...
class flow_reg(nn.Module):
    ...
    def __init__(self, size, ent=False, maxdisp=int(4), fac=1):
        ...
        flowy = np.tile(np.reshape(meshgrid[0], [1, 2 * maxdisp + 1, 2 * int(maxdisp // self.fac) + 1, 1, 1]),
                        (B, 1, 1, H, W))
        flowx = np.tile(np.reshape(meshgrid[1], [1, 2 * maxdisp + 1, 2 * int(maxdisp // self.fac) + 1, 1, 1]),
                        (B, 1, 1, H, W))
        self.register_buffer('flowx', torch.Tensor(flowx))
        self.register_buffer('flowy', torch.Tensor(flowy))

        self.pool3d = nn.MaxPool3d((self.wsize * 2 + 1, self.wsize * 2 + 1, 1), stride=1,
...
class VCN(nn.Module):
    ...
    def __init__(self, size, md=[4, 4, 4, 4, 4], fac=1., exp_unc=True):
        ...
        self.flow_reg64 = flow_reg([fdimb1 * size[0], size[1] // 64, size[2] // 64], ent=use_entropy,
                                   maxdisp=self.md[0], fac=self.fac)
        self.flow_reg32 = flow_reg([fdimb1 * size[0], size[1] // 32, size[2] // 32], ent=use_entropy,
                                   maxdisp=self.md[1])
        self.flow_reg16 = flow_reg([fdimb1 * size[0], size[1] // 16, size[2] // 16], ent=use_entropy,
                                   maxdisp=self.md[2])
        ...
        self.reg_modules = nn.ModuleList(
            [self.flow_reg64, self.flow_reg32, self.flow_reg16, self.flow_reg8, self.flow_reg4])
        self.oor_modules = nn.ModuleList(
            [self.dc6_convo, self.dc5_convo, self.dc4_convo, self.dc3_convo, self.dc2_convo])
    ...
    def corrf(self, refimg_fea, targetimg_fea, maxdisp, fac=1):
        if self.training:
            ...
            targetimg_fea = F.unfold(targetimg_fea, (2 * int(maxdisp) // fac + 1, 2 * maxdisp + 1),
                                     padding=(int(maxdisp) // fac, maxdisp)).view(b, c, 2 * int(maxdisp) // fac + 1,
                                                                                  2 * maxdisp + 1, h, w).permute(0, 1,
                                                                                                                 3, 2,
                                                                                                                 4,
                                                                                                                 5).contiguous()
            ...
            if refimg_fea.is_cuda:
                cost = Variable(
                    torch.cuda.FloatTensor(b, c, 2 * maxdisp + 1, 2 * int(maxdisp // fac) + 1, height, width)).fill_(
                    0.)  # b,c,u,v,h,w
            else:
                cost = Variable(
                    torch.FloatTensor(b, c, 2 * maxdisp + 1, 2 * int(maxdisp // fac) + 1, height, width)).fill_(
                    0.)  # b,c,u,v,h,w
    ...
    def cost_matching(self, up_flow, c1, c2, flowh, enth, level):
        ...
        if level == 0:
            ...
            flowh = torch.cat(
                (flowhh, F.upsample(flowh.detach() * 2, [flowhh.shape[2], flowhh.shape[3]], mode='bilinear')),
                1)  # b, k2--k2, h, w
    ...
    def affine(self, pref, flow, pw=1):
        ...
        ptar = pref + flow
        pw = 1
        pref = F.unfold(pref, (pw * 2 + 1, pw * 2 + 1), padding=(pw)).view(b, 2, (pw * 2 + 1) ** 2, lh, lw) - pref[:, :,
                                                                                                              np.newaxis]
        ptar = F.unfold(ptar, (pw * 2 + 1, pw * 2 + 1), padding=(pw)).view(b, 2, (pw * 2 + 1) ** 2, lh, lw) - ptar[:, :,
                                                                                                              np.newaxis]  # b, 2,9,h,w
        ...
        ppinv = torch.cat((prefprefT[:, 1, 1:], -prefprefT[:, 0, 1:], -prefprefT[:, 1:, 0], prefprefT[:, 0:1, 0]),
                          1).view(-1, 2, 2) / ppdet.clamp(1e-10, np.inf)[:, np.newaxis, np.newaxis]

        Affine = ptar.matmul(pref.permute(0, 2, 1)).matmul(ppinv)
        Error = (Affine.matmul(pref) - ptar).norm(2, 1).mean(1).view(b, 1, lh, lw)

        Avol = (Affine[:, 0, 0] * Affine[:, 1, 1] - Affine[:, 1, 0] * Affine[:, 0, 1]).view(b, 1, lh, lw).abs().clamp(
            1e-10, np.inf)
        exp = Avol.sqrt()
    ...
    def forward(self, im, disc_aux=None, disp_input=None):
        ...
        if not self.training or disc_aux[-1]:
            b, _, h, w = flow2.shape
            exp2, err2, _ = self.affine(get_grid(b, h, w)[:, 0].permute(0, 3, 1, 2).repeat(b, 1, 1, 1).clone(),
                                        flow2.detach(), pw=1)
            x = torch.cat((
        ...
        if not self.training or disc_aux[-1] == 2:
            ...
            if self.training:
                ...
                fscaled = 448. / H
            hp0o = \
            torch.cat([torch.arange(0, W, out=torch.cuda.FloatTensor()).view(1, -1).repeat(H, 1)[np.newaxis],  # 1,2,H,W
                       torch.arange(0, H, out=torch.cuda.FloatTensor()).view(-1, 1).repeat(1, W)[np.newaxis]], 0)[
                np.newaxis]
            ...
            if self.training:
                ...
                trans = trans + torch.Tensor(np.random.normal(loc=0., scale=5e-2, size=(bs, 3))).cuda() * trans.norm(2,
                                                                                                                     1)[
                                                                                                          :, np.newaxis]
            ...
            ## fg/bg segmentation
            # rigidity cost maps
            mcost00, mcost01, mcost1, mcost2, mcost3, mcost4, p3dmag, _ = compute_geo_costs(rot, trans, Ex, Kinv, hp0,
                                                                                            hp1, tau, Kinv_n=Kinv_n)
            # depth contrast cost
            ...
            # cost compute
            mcost00, mcost01, mcost1, mcost2, mcost3, mcost4, p3dmag, _ = compute_geo_costs(rot, trans, Ex, Kinv, hp0d,
                                                                                            hp1d, taud, Kinv_n=Kinv_n)
        ...
        if self.training:
            if disc_aux[-1] == 1:
                ...
                gt_expi, gt_expi_err, maskoe = self.affine_mask(
                    get_grid(b, 4 * h, 4 * w)[:, 0].permute(0, 3, 1, 2).repeat(b, 1, 1, 1), flowl0, pw=3)
                gt_exp = 1. / gt_expi[:, 0]

                loss = 0.1 * (dchange2[:, 0] - gt_dchange.log()).abs()[maskdc].mean()
                loss += 0.1 * (iexp2[:, 0] - gt_exp.log()).abs()[maskoe].mean()

                unc_loss = dc_unc[maskdc] + (dchange2[:, 0].detach() - gt_dchange.log()).pow(2)[maskdc] / dc_unc.exp()[
                    maskdc]
                ...
                loss = 0.01 * F.binary_cross_entropy_with_logits(fgmask[:, 0][valid_mask], fgmask_gt, size_average=True,
                                                                 weight=weights)
                ...
                from utils.detlib import distance2mask, draw_umich_gaussian, gaussian_radius, get_polarmask, polar_reg, \
                    pose_reg
                obj_idx = disc_aux[2][:, :, :, 7] * (
                            (m3d_gt * 100 > 1) & (disc_aux[2][:, :, :, 0] < 100)).float()  # remove background
                ...
                for i in range(bs):
                    if pdist_mask[i].sum() > 0:
                        contour = distance2mask(pdist_ct[i][pdist_mask[i]], pdist_gt[i][pdist_mask[i]], angles,
                                                im.shape[2:])
                        ...
                        contour = distance2mask(pdist_ct[i][pdist_mask[i]], pdist[i][pdist_mask[i]], angles,
                                                im.shape[2:])
                ...
                m3d_gt = torch.cat([disc_aux[2][:, :, :, 7], (m3d_gt * 1000).clamp(0, 255),
                                    ...
                                    (100 * F.interpolate(log_dratio, [H, W], mode='bilinear')[:, 0].exp()).clamp(0,
                                                                                                                 255),
