...
if 'expansion' in args.stage:
    ...
    if '2015' in args.stage:
        ...
        loader_kitti15_sc = dd.myImageFloder(iml0, iml1, flowl0, shape=datashape, scale=1, order=0, prob=0.5,
                                             disp0=disp0, disp1=disp1, calib=calib)
    else:
        ...
        for impath in iml0:
            ...
            disp0.append('%s/Sintel/disparities/%s_%s/frame_%04d.png' % (
            impath.rsplit('/', 2)[0], seqname1, seqname2, framename + 1))
            disp1.append('%s/Sintel/disparities/%s_%s/frame_%04d.png' % (
            impath.rsplit('/', 2)[0], seqname1, seqname2, framename + 2))
            calib.append('%s/Sintel/camdata_left/%s_%s/frame_%04d.cam' % (
            impath.rsplit('/', 2)[0], seqname1, seqname2, framename + 1))
        loader_sintel_sc = dd.myImageFloder(iml0, iml1, flowl0, shape=datashape, scale=1, order=1, noise=0, disp0=disp0,
                                            disp1=disp1, calib=calib)
elif 'seg' in args.stage:
    from dataloader import exploader as dd
    from dataloader import sceneflowlist as lsf
    ...
    loader_kitti15_sc = dd.myImageFloder(iml0, iml1, flowl0, shape=datashape, scale=1, order=0, prob=0.5, disp0=disp0,
                                         disp1=disp1, calib=calib)
else:  # flow
...
if 'expansion' in args.stage:
    if '2015' in args.stage:
        ...
        data_inuse = torch.utils.data.ConcatDataset(
            [loader_driving_sc] * 200 + [loader_monkaa_sc] * 100 + [loader_things_sc] * 40)  # no kitti sintel
    ...
    if args.stage == 'segsf':
        ...
        data_inuse = torch.utils.data.ConcatDataset(
            [loader_driving_sc] * 200 + [loader_monkaa_sc] * 100 + [loader_things_sc] * 40)
    ...
    data_inuse = torch.utils.data.ConcatDataset(
        [loader_kitti15] * 200 * 6 + [loader_hd1k] * 40 * 6 + [loader_sintel] * 150 * 6 + [loader_chairs] * 2 * 6 + [
            loader_things] * 6)
    for i in data_inuse.datasets:
        i.black = True
        i.cover = True
    ...
    data_inuse = torch.utils.data.ConcatDataset(
        [loader_kitti12] * 2700 + [loader_kitti15] * 2700 + [loader_sintel] * 600 + [loader_chairs] * 12 + [
            loader_things] * 6 + [loader_hd1k] * 900 + [loader_driving] * 50 + [loader_monkaa] * 25 + [
            loader_viper] * 70)
...
model = VCN([batch_size // ngpus] + data_inuse.datasets[0].shape[::-1],
            md=[int(4 * (args.maxdisp / 256)), 4, 4, 4, 4], fac=args.fac,
            exp_unc=args.loadmodel is None or not ('kitti' in args.loadmodel))
...
if args.loadflow is not None:
    ...
    pretrained_dict['state_dict'] = {k: v for k, v in pretrained_dict['state_dict'].items() if
                                     'f_modules' in k or 'p_modules' in k or 'oor_modules' in k or 'fuse_modules' in k}
    model.load_state_dict(pretrained_dict['state_dict'], strict=False)
...
def train(imgL, imgR, flowl0, imgAux, intr, imgoL, imgoR, occp, RT01):
    ...
    with autocast(enabled=mix_precision):
        output = model(torch.cat(groups, 0), [flowl0, mask, imgAux, intr, imgoL, imgoR, occp, RT01, exp_flag],
...
def main():
    ...
    TrainImgLoader = torch.utils.data.DataLoader(
        data_inuse,
        batch_size=batch_size, num_workers=int(worker_mul * batch_size), drop_last=True, worker_init_fn=_init_fn,
    ...
    for batch_idx, databatch in enumerate(TrainImgLoader):
        ...
        if args.local_rank == 0:
            ...
            if total_iters % 100 == 0:
                ...
                if len(np.asarray(vis['gt'])) > 0:
                    log.add_histogram('train/gt_hist', np.asarray(vis['gt']).reshape(-1, 3)[
                                                           np.asarray(vis['gt'])[:, :, :, -1].flatten().astype(bool)][:,
                                                       :2], total_iters)
                gu = vis['gt'][0, :, :, 0];
                gv = vis['gt'][0, :, :, 1]
                ...
                add_image(log, 'train/gt0', flow_to_image(
                    np.concatenate((gu[:, :, np.newaxis], gv[:, :, np.newaxis], mask[:, :, np.newaxis]), -1))[
                    np.newaxis], total_iters)
                add_image(log, 'train/output2', flow_to_image(vis['output2'][0].transpose((1, 2, 0)))[np.newaxis],
                          total_iters)
                add_image(log, 'train/output3', flow_to_image(vis['output3'][0].transpose((1, 2, 0)))[np.newaxis],
                          total_iters)
                add_image(log, 'train/output4', flow_to_image(vis['output4'][0].transpose((1, 2, 0)))[np.newaxis],
                          total_iters)
                add_image(log, 'train/output5', flow_to_image(vis['output5'][0].transpose((1, 2, 0)))[np.newaxis],
                          total_iters)
                add_image(log, 'train/output6', flow_to_image(vis['output6'][0].transpose((1, 2, 0)))[np.newaxis],
                          total_iters)
                if 'expansion' in args.stage:
        ...
        if (total_iters + 1) % 2000 == 0:
            if args.local_rank == 0:
                ...
                save_dict = collections.OrderedDict({k: v for k, v in save_dict.items() if
                                                     ('reg_modules' not in k or 'conv1' in k) and (
                                                                 'grid' not in k) and ('flow_reg' not in k) and (
                                                                 'midas' not in k)})
                torch.save({
                    'iters': total_iters,
